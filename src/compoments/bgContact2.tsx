...
interface DitherShaderProps {
  ...
  /** Custom color palette array for custom mode (du plus sombre au plus clair) */
  customPalette?: string[];
  /** 0..1 — intensité du tramage entre paliers en mode custom multi-couleurs */
  customDitherAmount?: number;
  ...
  /** Respecte prefers-reduced-motion (coupe l'animation) */
...
export const DitherShader: React.FC<DitherShaderProps> = ({
  ...
  const applyDithering = useCallback(
    (
      ...
      const paletteCount = parsedCustomPalette.length;
          ...
          switch (colorMode) {
            ...
            case "custom": {
              if (paletteCount === 2) {
                ...
                // Quantification sur N paliers AVEC tramage Bayer entre paliers.
                // On place la luminance sur l'échelle [0, N-1], puis on
                // décale via (ditherThreshold - 0.5) pour que les pixels
                // voisins basculent d'un palier à l'autre → effet dither.
                const scaled = clamp(luminance, 0, 1) * (paletteCount - 1);
                const dither = (ditherThreshold - 0.5) * customDitherAmount;
                const paletteIndex = clamp(
                  Math.round(scaled + dither),
                  0,
                  paletteCount - 1,
                );
  ...
  useEffect(() => {
    ...
    const isAnimated = animated && !prefersReduced;

...
type PixelatedCanvasProps = {
  ...
  fadeOnLeave?: boolean;
  fadeSpeed?: number;
  /** Désactive l'animation si l'utilisateur a activé prefers-reduced-motion. */
...
const samplesRef = React.useRef<Sample[]>([]);  
  const dimsRef = React.useRef<{ width: number; height: number; dot: number } | null>(
    null
  );
  ...
  const sizeRef = React.useRef({ w: width, h: height });
  ...
  React.useEffect(() => {
    ...
    const isInteractive = interactive && !prefersReduced;
    ...
    const compute = () => {
      ...
      const displayWidth = sizeRef.current.w || img.naturalWidth;
      const displayHeight = sizeRef.current.h || img.naturalHeight;
    ...
    const drawStatic = () => {
      ...
      for (const s of samples) {
        ...
        if (shape === "circle") {
          ctx.beginPath();
          ctx.arc(s.x + cellSize / 2, s.y + cellSize / 2, dims.dot / 2, 0, Math.PI * 2);
          ctx.fill();
        } else {
          ctx.fillRect(
    ...
    img.onerror = () => {
      console.error("Échec du chargement de l'image pour PixelatedCanvas:", src);
    };
  ...
  return (
    <canvas
      ref={canvasRef}
      className={className}
      aria-label="Rendu pixelisé d'une image"
      role="img"
    />
  );
};